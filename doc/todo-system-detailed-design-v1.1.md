...
1. Markdown 格式扩展：子任务下方可追加 `start:` / `due:` 前缀行，沿用现有 `note:` / `link:` 的缩进前缀约定。
2. 数据模型扩展：`Subtask` 增加 `start?: string` 与 `due?: string`。
3. UI 扩展：`TaskEditor` 中为每个子任务增加开始/截止时间输入框。
4. 解析/序列化同步更新：扫描器与序列化器识别并输出子任务日期。
5. 状态推断保持不变：子任务完成状态仍是主任务状态的唯一依据；子任务日期仅用于展示与提醒（可选扩展）。

### 2. 备注支持实时 Markdown 预览

1. 任务备注和子任务备注的编辑区均支持「编辑 / 预览」切换。
2. 预览侧使用已有的 `react-markdown + remark-gfm` 渲染，保持与最终显示效果一致。
3. 默认显示编辑区；点击预览标签即时渲染当前输入内容，无需保存。
4. 不引入额外依赖。
...
备注/链接/日期段落均以该子任务的缩进级别 +2 空格开始，遇到下一个 `- [ ]`（同级或更深）或非属性行时结束。
...
type ParsedSubtask = {
  ...
  completed: boolean;
  completed_at?: string;
  start?: string;          // 开始日期（v1.1 新增）
  due?: string;            // 截止日期（v1.1 新增）
  note?: string;
  links?: Link[];
...
- 开始时间（可选）
- 截止时间（可选）
- 备注（可选，Markdown 编辑器，支持实时预览）
- 链接（可选，URL 输入框）
- 子任务（可选，逐行输入，每个子任务可独立设置开始/截止时间和备注预览）
- 重复规则（可选，下拉选择）

**编辑任务：** 点击任务卡片 → 进入详情页（或展开)。所有字段均可编辑。备注字段支持「编辑 / 预览」切换，预览使用 `react-markdown` 实时渲染。
...
interface MarkdownPreviewProps {
  source: string;               // 要渲染的 Markdown 字符串
  className?: string;
}
...
| 功能 | 状态 | 说明 |
|---|---|---|
| 子任务开始/截止日期 | ✅ | Markdown 格式、解析/序列化、TaskEditor UI 同步支持 |
| 备注实时 Markdown 预览 | ✅ | 任务备注 + 子任务备注均支持编辑/预览切换，使用 react-markdown 渲染 |
| 回收站查看/恢复 UI | ❌ | 暂不实现 |
| 离线完整支持 | ✅ | 待写入队列展示 + 手动重试 + 冲突提示条 |
...
Phase 3: 交互功能
  ...
  ✅ 子任务开始/截止日期编辑（v1.1 新增）
  ✅ 备注实时 Markdown 预览（v1.1 新增）
...
Phase 5: 通知与优化
  ...
  ❌ 回收站（暂不实现，删除时二次确认即可）
  ✅ 离线完整支持（待写入队列展示 + 手动重试 + 冲突提示，v1.1 完成）
...
- 按钮位于「新建分组」按钮旁边，与搜索/过滤条同处一个顶部卡片区域。
