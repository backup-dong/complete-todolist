...
| 移动端打开侧边栏后关闭，顶部和底部也就是手机导航栏还是灰色的 | 是 | 补充 `viewport-fit=cover` 与 `black-translucent` 状态栏配置，`ThemeProvider` 动态同步 `theme-color`，并在移动端头部/底部/抽屉/弹窗添加 `env(safe-area-inset-*)` 内边距 |
